...
def now_jakarta():
    """Waktu sekarang di zona Asia/Jakarta (aware)."""
...
EMBED_MODEL_ID    = os.getenv("BEDROCK_EMBEDDINGS_MODEL_ID", "amazon.titan-embed-text-v1")
...
class ChatManager:
    ...
    def create_new_session(self, title: str = None, user_id: str = "default") -> str:
        ...
        if not title:
            title = f"Chat {now_jakarta().strftime('%d/%m %H:%M')}"
    ...
    def export_chat_history(self, session_id: str = None) -> Dict:
        ...
        return {
            'sessions': sessions,
            'messages': messages,
            'exported_at': now_jakarta().isoformat()
        }
...
class FinancialAI:
    ...
    def create_pdf_report(self, content: str, title: str = "Financial Analysis Report") -> BytesIO:
        ...
        pdf.drawString(50, height - 70, f"Generated on: {now_jakarta().strftime('%Y-%m-%d %H:%M:%S')}")
...
def render_sidebar():
    ...
    if st.sidebar.button("📥 Export All Chats"):
        ...
        st.sidebar.download_button(
            label="💾 Download JSON",
            data=export_json,
            file_name=f"chat_history_{now_jakarta().strftime('%Y%m%d_%H%M%S')}.json",
            ...
            with st.chat_message("assistant"):
                ...
                if st.button(f"📥 Download PDF", key=f"download_{message['timestamp']}"):
                    pdf_buffer = st.session_state.ai.create_pdf_report(
                        message['content'],
                        f"Financial Analysis - {now_jakarta().strftime('%Y%m%d_%H%M%S')}"
                    )
                    st.download_button(
                        label="📄 Download Report",
                        data=pdf_buffer,
                        file_name=f"financial_analysis_{now_jakarta().strftime('%Y%m%d_%H%M%S')}.pdf",
